...
public class GetStringMethodClassOperate {
            ...
            if (Tools.protosMap.size() > 0) {
                for (String protoPath : Tools.protosMap.keySet()) {
                    ...
                    for (String innerClassName : list) {
                        ...
                        if (innerCtClass.hasAnnotation(GetStringMethod.class)) {
                            GetStringMethod annotation = (GetStringMethod) innerCtClass.getAnnotation(GetStringMethod.class);
                            String[] value = annotation.value();
                            // 创建新的方法
                            CtMethod newMethod = new CtMethod(pool.get("java.lang.String"), "getString", new CtClass[]{pool.get("java.lang.String")}, innerCtClass);
